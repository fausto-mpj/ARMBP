...
# Struct
struct ARHMM <: AbstractHMM{true}
    init::Vector{Vector{Float64}}
    trans::Vector{Matrix{Float64}}
	dists::Vector{Matrix{Float64}}
end

# Função para o vetor de distribuição inicial
function initialization(hmm::ARHMM, control::Integer)
	return(hmm.init[control])
end
initialization(hmm::ARHMM) = hmm.init

# Função para pegar do vetor de matrizes |Xₜ| x |Xₜ|, com |Uₜ| elementos, a matriz de transição associada ao controle uₜ no tempo t.
function transition_matrix(hmm::ARHMM, control::Integer)
	return(hmm.trans[control])
end


# Função para pegar do vetor de matrizes |Uₜ| x |Yₜ|, com |Xₜ| elementos, a distribuição categórica de |Yₜ| termos associada ao controle uₜ e binário xₜ no tempo t
function obs_distributions(hmm::ARHMM, control::Integer)
	return([Categorical(hmm.dists[i][control,:]) for i in 1:size(hmm, control)])
end
